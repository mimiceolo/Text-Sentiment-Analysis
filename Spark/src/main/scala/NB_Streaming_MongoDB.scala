...
object NB_Streaming_MongoDB {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val conf = new SparkConf()
            .setAppName("Sentiment Analysis Streaming NB - MongoDB")
            .set("spark.streaming.kafka.maxRatePerPartition", "1000")

        val ssc = new StreamingContext(conf, Seconds(10))
        ssc.checkpoint("hdfs://localhost:9000/user/spark/checkpoints_nb_mongo")
        ...
        val kafkaParams = Map[String, Object](
            ...
            "group.id" -> "spark-nb-streaming-mongo",
        ...
        stream.foreachRDD { rdd =>
            if (!rdd.isEmpty()) {
                ...
                println(s"\n[NB-MongoDB] Processing batch $batchId with ${rdd.count()} records")
                ...
                try {
                    ...
                    if (tweetsDF.count() > 0) {
                        ...
                        println(s"[NB-MongoDB] Batch $batchId - Accuracy: $accuracy, F1: $f1Score")
                        ...
                        try {
                            if (!predictionsList.isEmpty) {
                                ...
                                println(s"[NB-MongoDB] ✓ Wrote ${predictionsList.size()} predictions to MongoDB")
                            }
                        } catch {
                            case e: Exception =>
                                println(s"[NB-MongoDB] ⚠ MongoDB write failed (non-fatal): ${e.getMessage}")
                        }
                        ...
                        try {
                            ...
                            println(s"[NB-MongoDB] ✓ Wrote batch metrics to MongoDB")
                        } catch {
                            case e: Exception =>
                                println(s"[NB-MongoDB] ⚠ Metrics write failed (non-fatal): ${e.getMessage}")
                        }
                        ...
                        println(s"[NB-MongoDB] ✓ Batch complete in ${processingTime}ms")
                    }
                } catch {
                    case e: Exception =>
                        println(s"[NB-MongoDB] ✗ Error processing batch: ${e.getMessage}")
                        e.printStackTrace()
                }
            }
        }
